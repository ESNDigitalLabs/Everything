...
eList = ["a", "b", "c"]
fList = ["c", "d", "e"]
listEF = [eList[i] for i in range(len(eList)) if eList[i] in fList]
print(listEF)
...
cList = [1, 2, 3, 4, 5, 12, 13, 14, 15, 20, 21]
dList = [6, 7, 8, 9, 10, 4, 5, 12, 13, 14, 30]
joinedList = cList + dList
print(f"The joined list\n{joinedList}")

#print(f"The common numbers in both list c and d using list comprehension:\n{commonNums}\n")

listEF = [cList[i] for i in range(len(cList)) if cList[i] in dList]
print(f"These are the commom numbers\n{listEF}")
